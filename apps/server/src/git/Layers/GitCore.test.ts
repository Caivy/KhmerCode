...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "khmercode-git-core-test-",
});
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "khmercode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "khmercode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "khmercode/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "khmercode/tmp-working",
          newBranch: "khmercode/feat/session",
        });

        expect(renamed.branch).toBe("khmercode/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "khmercode/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "khmercode/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("khmercode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "khmercode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "khmercode/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "khmercode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "khmercode/tmp-working" });
        ...
        expect(renamed.branch).toBe("khmercode/feat/session-2");
      }),
    );
