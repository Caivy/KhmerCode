...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "khmercode-codex-text-generation-test-",
    }),
  ),
    ...
    yield* fs.writeFileString(
      ...
      process.platform === "win32"
        ? [
            ...
            "goto parse_args",
            ":after_args",
            'set "stdin_file=%TEMP%\\khmercode-fake-codex-%RANDOM%-%RANDOM%.txt"',
            'more > "%stdin_file%"',
            'if "%T3_FAKE_CODEX_REQUIRE_IMAGE%"=="1" if not defined seen_image (',
            "  >&2 echo missing --image input",
            '  del "%stdin_file%" >nul 2>nul',
            "  exit /b 2",
            ")",
            "if defined T3_FAKE_CODEX_STDIN_MUST_CONTAIN (",
            '  findstr /C:"%T3_FAKE_CODEX_STDIN_MUST_CONTAIN%" "%stdin_file%" >nul || (',
            "    >&2 echo stdin missing expected content",
            '    del "%stdin_file%" >nul 2>nul',
            "    exit /b 3",
            "  )",
            ")",
            "if defined T3_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN (",
            '  findstr /C:"%T3_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN%" "%stdin_file%" >nul && (',
            "    >&2 echo stdin contained forbidden content",
            '    del "%stdin_file%" >nul 2>nul',
            "    exit /b 4",
            "  )",
            ")",
            "if defined T3_FAKE_CODEX_STDERR >&2 echo %T3_FAKE_CODEX_STDERR%",
            "if defined output_path powershell -NoProfile -NonInteractive -Command \"$value = if ($env:T3_FAKE_CODEX_OUTPUT_B64) { $env:T3_FAKE_CODEX_OUTPUT_B64 } else { 'e30=' }; [System.IO.File]::WriteAllBytes($env:output_path, [System.Convert]::FromBase64String($value))\"",
            'del "%stdin_file%" >nul 2>nul',
            "if defined T3_FAKE_CODEX_EXIT_CODE exit /b %T3_FAKE_CODEX_EXIT_CODE%",
            "exit /b 0",
            "",
          ].join("\r\n")
        : [
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "khmercode-codex-text-" });
      const binDir = yield* makeFakeCodexBinary(tempDir);
      const codexBinaryPath =
        process.platform === "win32" ? `${binDir}\\codex.cmd` : `${binDir}/codex`;
      const previousPath = process.env.PATH;
      const previousCodexBinaryPath = process.env.CODEX_BINARY_PATH;
      ...
      yield* Effect.sync(() => {
        ...
        process.env.CODEX_BINARY_PATH = codexBinaryPath;
    ...
    (previous) =>
      Effect.sync(() => {
        ...
        if (previous.previousCodexBinaryPath === undefined) {
          delete process.env.CODEX_BINARY_PATH;
        } else {
          process.env.CODEX_BINARY_PATH = previous.previousCodexBinaryPath;
        }
...
if (process.platform === "win32") {
  it.skip("CodexTextGenerationLive is not exercised on Windows test runners", () => {});
} else {
