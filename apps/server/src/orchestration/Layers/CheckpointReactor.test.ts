...
import type { ProviderKind, ProviderRuntimeEvent, ProviderSession } from "@khmercode/contracts";
import {
  CommandId,
...
function createGitRepository() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "khmercode-checkpoint-handler-"));
...
function gitShowFileAtRef(cwd: string, ref: string, filePath: string): string {
  return runGit(cwd, ["show", `${ref}:${filePath}`]).replace(/\r\n/g, "\n");
}
...
describe("CheckpointReactor", () => {
  ...
  async function createHarness(options?: {
    ...
    const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
      prefix: "khmercode-checkpoint-reactor-test-",
    });
  ...
  it("continues processing runtime events after a single checkpoint runtime failure", async () => {
    const nonRepositorySessionCwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "khmercode-checkpoint-runtime-non-repo-"),
    );
