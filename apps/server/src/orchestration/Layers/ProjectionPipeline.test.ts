...
const BaseTestLayer = makeProjectionPipelinePrefixedTestLayer(
  "khmercode-projection-pipeline-test-",
);
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("khmercode-base-")))(
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("khmercode-projection-attachments-safe-")),
...
it.layer(
  Layer.fresh(
    makeProjectionPipelinePrefixedTestLayer("khmercode-projection-attachments-overwrite-"),
  ),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(
    makeProjectionPipelinePrefixedTestLayer("khmercode-projection-attachments-rollback-"),
  ),
)("OrchestrationProjectionPipeline", (it) => {
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.makeUnsafe(
            "refs/khmercode/checkpoints/thread-revert-files/turn/1",
          ),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.makeUnsafe(
            "refs/khmercode/checkpoints/thread-revert-files/turn/2",
          ),
          status: "ready",
          files: [],
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("khmercode-projection-attachments-revert-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("khmercode-projection-attachments-delete-")),
)("OrchestrationProjectionPipeline", (it) => {
      ...
      Effect.gen(function* () {
        ...
        yield* appendAndProject({
          ...
          payload: {
            ...
            checkpointTurnCount: 1,
            checkpointRef: CheckpointRef.makeUnsafe(
              "refs/khmercode/checkpoints/thread-conflict/turn/1",
            ),
            status: "ready",
            files: [],
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 1,
          checkpointRef: CheckpointRef.makeUnsafe(
            "refs/khmercode/checkpoints/thread-revert/turn/1",
          ),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 2,
          checkpointRef: CheckpointRef.makeUnsafe(
            "refs/khmercode/checkpoints/thread-revert/turn/2",
          ),
          status: "ready",
          files: [],
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "khmercode-projection-pipeline-restart-",
        }),
        NodeServices.layer,
      ),
    ),
...
const engineLayer = it.layer(
  OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "khmercode-projection-pipeline-engine-dispatch-",
      }),
    ),
    Layer.provideMerge(NodeServices.layer),
  ),
  ...
  it.effect("projects persist updated scripts from project.meta.update", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(projectRows, [
        {
          scriptsJson:
            '[{"id":"script-1","name":"Build","command":"pnpm build","icon":"build","runOnWorktreeCreate":false}]',
          defaultModel: "gpt-5",
        },
      ]);
    }),
