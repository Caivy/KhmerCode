...
import type { OrchestrationReadModel } from "@khmercode/contracts";
...
import { CliConfig, khmercodeCli, recordStartupHeartbeat, type CliConfigShape } from "./main";
...
const resolveCliPath = (value: string) => path.resolve(value);
const resolveCliStateDir = (value: string) => path.join(resolveCliPath(value), "dev");
...
const testLayer = Layer.mergeAll(
  Layer.succeed(CliConfig, {
    cwd: "/tmp/khmercode-test-workspace",
    fixPath: Effect.void,
...
const runCli = (
  ...
  env: Record<string, string> = { KHMERCODE_NO_BROWSER: "true" },
) => {
  return Command.runWith(khmercodeCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.baseDir, resolveCliPath("/tmp/khmercode-cli-home"));
      assert.equal(resolvedConfig?.stateDir, resolveCliStateDir("/tmp/khmercode-cli-home"));
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ...
        KHMERCODE_PORT: "4999",
        KHMERCODE_HOST: "100.88.10.4",
        KHMERCODE_HOME: "/tmp/khmercode-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        KHMERCODE_NO_BROWSER: "true",
        KHMERCODE_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.baseDir, resolveCliPath("/tmp/khmercode-env-home"));
      assert.equal(resolvedConfig?.stateDir, resolveCliStateDir("/tmp/khmercode-env-home"));
  ...
  it.effect("prefers --mode over KHMERCODE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over KHMERCODE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        KHMERCODE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        KHMERCODE_MODE: "desktop",
        KHMERCODE_LOG_WS_EVENTS: "false",
        KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        KHMERCODE_NO_BROWSER: "true",
      });
