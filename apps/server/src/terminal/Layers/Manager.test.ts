...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "khmercode-terminal-"));
  ...
  it("retries with fallback shells when preferred shell spawn fails", async () => {
    ...
    expect(ptyAdapter.spawnInputs[1]?.shell).not.toBe("/definitely/missing-shell -l");

    if (process.platform === "win32") {
      expect(
        ptyAdapter.spawnInputs.some((input) => input.shell.toLowerCase().includes("cmd")),
      ).toBe(true);
    } else {
      expect(
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.KHMERCODE_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          KHMERCODE_PROJECT_ROOT: "/repo",
          KHMERCODE_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.KHMERCODE_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.KHMERCODE_WORKTREE_PATH).toBe("/repo/worktree-a");
