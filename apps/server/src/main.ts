...
const CliEnvConfig = Config.all({
  mode: Config.string("KHMERCODE_MODE").pipe(
    Config.option,
    Config.map(
  ...
  port: Config.port("KHMERCODE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("KHMERCODE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  homeDir: Config.string("KHMERCODE_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("KHMERCODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("KHMERCODE_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("KHMERCODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
  ...
  Effect.gen(function* () {
    ...
    if (!config.devUrl && !config.staticDir) {
      yield* Effect.logWarning(
        ...
        {
          hint: "Run `pnpm --dir apps/web build` or set VITE_DEV_SERVER_URL for dev mode.",
        },
      );
    }
    ...
    yield* Effect.logInfo("KhmerCode running", {
      ...safeConfig,
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to KHMERCODE_LOG_WS_EVENTS).",
  ),
  Flag.withAlias("log-ws-events"),
  Flag.optional,
);

export const khmercodeCli = Command.make("khmercode", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  homeDir: homeDirFlag,
  ...
  Command.withDescription("Run the KhmerCode server."),
