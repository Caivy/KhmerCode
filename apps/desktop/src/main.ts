...
import type { ContextMenuItem } from "@khmercode/contracts";
import { NetService } from "@khmercode/shared/Net";
import { RotatingFileSink } from "@khmercode/shared/logging";
...
const BASE_DIR = process.env.KHMERCODE_HOME?.trim() || Path.join(OS.homedir(), ".khmercode");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SCHEME = "khmercode";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "KhmerCode (Dev)" : "KhmerCode";
const APP_USER_MODEL_ID = "com.khmercode.khmercode";
const USER_DATA_DIR_NAME = isDevelopment ? "khmercode-dev" : "khmercode";
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { khmercodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.khmercodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.KHMERCODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("KhmerCode failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.KHMERCODE_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `KhmerCode ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/KhmerCode` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`khmercode`) to avoid spaces
 * and parentheses in platform-specific config paths.
 */
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.KHMERCODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
...
function backendEnv(): NodeJS.ProcessEnv {
  return {
    ...process.env,
    KHMERCODE_MODE: "desktop",
    KHMERCODE_NO_BROWSER: "1",
    KHMERCODE_PORT: String(backendPort),
    KHMERCODE_HOME: BASE_DIR,
    KHMERCODE_AUTH_TOKEN: backendAuthToken,
  };
}
...
async function bootstrap(): Promise<void> {
  ...
  process.env.KHMERCODE_DESKTOP_WS_URL = backendWsUrl;
