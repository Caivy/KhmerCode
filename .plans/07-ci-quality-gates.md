...
- pnpm + Turbo setup in CI.

## Proposed Changes

1. Add `ci.yml` with jobs:
   - Setup pnpm and Node environment
   - Install deps
   - `pnpm lint`
   - `pnpm typecheck`
   - `pnpm test`
2. Add separate optional job for `pnpm run smoke-test` (desktop/Electron).
3. Configure caching for pnpm/Turbo as appropriate.
