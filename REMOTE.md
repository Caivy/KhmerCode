...
Use this when you want to open KhmerCode from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The KhmerCode CLI accepts the following configuration options, available either as CLI flags or environment variables:

| CLI flag                | Env var                | Notes                              |
| ----------------------- | ---------------------- | ---------------------------------- |
| `--mode <web\|desktop>` | `KHMERCODE_MODE`       | Runtime mode.                      |
| `--port <number>`       | `KHMERCODE_PORT`       | HTTP/WebSocket port.               |
| `--host <address>`      | `KHMERCODE_HOST`       | Bind interface/address.            |
| `--base-dir <path>`     | `KHMERCODE_HOME`       | Base directory.                    |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL`  | Dev web URL redirect/proxy target. |
| `--no-browser`          | `KHMERCODE_NO_BROWSER` | Disable auto-open browser.         |
| `--auth-token <token>`  | `KHMERCODE_AUTH_TOKEN` | WebSocket auth token.              |
...
pnpm --dir apps/server start -- --host 0.0.0.0 --port 3773 --auth-token "$TOKEN" --no-browser
```
...
TOKEN="$(openssl rand -hex 24)"
pnpm --dir apps/server start -- --host "$(tailscale ip -4)" --port 3773 --auth-token "$TOKEN" --no-browser
```
