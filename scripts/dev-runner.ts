...
import { NetService } from "@khmercode/shared/Net";
...
export const DEFAULT_HOME_DIR = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".khmercode"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@khmercode/contracts",
    "--filter=@khmercode/web",
    "--filter=khmercode",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=khmercode"],
  "dev:web": ["run", "dev", "--filter=@khmercode/web"],
  "dev:desktop": [
    "run",
    "dev",
    "--filter=@khmercode/desktop",
    "--filter=@khmercode/web",
    "--parallel",
  ],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("KHMERCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("KHMERCODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid KHMERCODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `KHMERCODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric KHMERCODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed KHMERCODE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      KHMERCODE_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      KHMERCODE_HOME: resolvedBaseDir,
    };

    if (host !== undefined) {
      output.KHMERCODE_HOST = host;
    }

    if (authToken !== undefined) {
      output.KHMERCODE_AUTH_TOKEN = authToken;
    } else {
      delete output.KHMERCODE_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.KHMERCODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.KHMERCODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.KHMERCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.KHMERCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.KHMERCODE_MODE = "web";
      delete output.KHMERCODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read KHMERCODE_PORT_OFFSET/KHMERCODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("KHMERCODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("KHMERCODE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KHMERCODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.KHMERCODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  homeDir: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all KhmerCode data (equivalent to KHMERCODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("KHMERCODE_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to KHMERCODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("KHMERCODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to KHMERCODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("KHMERCODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("KHMERCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to KHMERCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("KHMERCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to KHMERCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("KHMERCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to KHMERCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("KHMERCODE_PORT")),
  ),
