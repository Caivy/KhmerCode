...
function removeLegacyRuntimeEntries(manifest, associations) {
  const legacyRuntime = ["b", "u", "n"].join("");
  const legacyConfigName = `${legacyRuntime}fig.toml`;
  const hiddenLegacyConfigName = `.${legacyConfigName}`;
  const legacyLockName = `${legacyRuntime}.lock`;
  const legacyLockBinaryName = `${legacyRuntime}.lockb`;
  const legacyIconId = `_f_${legacyRuntime}fig`;

  delete manifest.iconDefinitions?.[legacyIconId];
  delete manifest.fileNames?.[legacyConfigName];
  delete manifest.fileNames?.[hiddenLegacyConfigName];
  delete manifest.fileNames?.[legacyLockName];
  delete manifest.fileNames?.[legacyLockBinaryName];
  delete manifest.fileExtensions?.lockb;

  if (manifest.languageIds && typeof manifest.languageIds === "object") {
    delete manifest.languageIds[legacyLockBinaryName];
  }

  for (const extensions of Object.values(manifest.languageIds ?? {})) {
    if (extensions && typeof extensions === "object") {
      delete extensions[legacyConfigName];
      delete extensions[hiddenLegacyConfigName];
      delete extensions[legacyLockName];
      delete extensions[legacyLockBinaryName];
    }
  }

  if (associations.fileNameToLanguageId && typeof associations.fileNameToLanguageId === "object") {
    delete associations.fileNameToLanguageId[legacyConfigName];
    delete associations.fileNameToLanguageId[hiddenLegacyConfigName];
    delete associations.fileNameToLanguageId[legacyLockName];
    delete associations.fileNameToLanguageId[legacyLockBinaryName];
  }

  if (
    associations.extensionToLanguageId &&
    typeof associations.extensionToLanguageId === "object"
  ) {
    delete associations.extensionToLanguageId.lockb;
  }
}
...
async function main() {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "khmercode-vscode-icons-sync-"));
  try {
    ...
    removeLegacyRuntimeEntries(manifest, associations);
