...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `khmercode` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@khmercode/shared/git`, `@khmercode/shared/DrainableWorker`) — no barrel index.
